...
function App() {
  ...
  return <div className="drawer drawer-end"><input id="my-drawer-4" type="checkbox" className="drawer-toggle" /><div className="drawer-content"><RoutesLayout /></div><Toast /><div className="drawer-side z-30"><label
    htmlFor="my-drawer-4"
    aria-label="close sidebar"
    className="drawer-overlay"
  /><ul className="hidden sm:block menu bg-base-200 text-base-content min-h-full w-[500px] p-4"><CartDesktopManage /></ul><ul className="sm:hidden flex gap-4 z-10 pt-12 relative menu bg-base-200 text-base-content min-h-full w-80 p-4">{(Auth?.isLogged ? PHONE_MENU_DATA.filter((item) => item.loggedIn === true) : PHONE_MENU_DATA.filter((item) => item.loggedIn === false)).map((item, index) => <li key={`${item.label}-${item.link}-${index}`}><NavLink
    to={item.link}
    className={({ isActive }) => `phone-drawer-menu-btn ${isActive ? "bg-black text-white" : ""}`}
  ><img
    src={item.svg}
    alt={item.label}
    className="w-6 h-6 inline-block mr-2"
  />{item.label}</NavLink></li>)}{userData?.userRole === 2 && isLoggedIn && <li><NavLink
    to={"/admin"}
    className={({ isActive }) => `phone-drawer-menu-btn ${isActive ? "bg-black text-white" : ""}`}
  ><img
    src="/admin-login.svg"
