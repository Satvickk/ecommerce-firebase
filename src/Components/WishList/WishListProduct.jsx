...
export default function WishListProduct() {
  ...
  return <section className="w-full bg-white border-b-4 border-black py-16"><div className="max-w-7xl mx-auto px-4 sm:px-8"><div className="flex items-center gap-4 mb-8 pb-4 border-b-4 border-black"><span className="bg-swiss-accent text-white px-3 py-1 text-xs font-black uppercase tracking-widest">
            ...
            MY WISHLIST
          </h1></div>{Wishlist.selectedProducts.length > 0 ? <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">{Wishlist.selectedProducts.map((item, index) => <ProductCard key={item.docId || index} data={item} />)}</div> : <div className="border-4 border-black p-12 text-center bg-swiss-muted"><h3 className="text-xl font-black uppercase tracking-tight mb-2">NO WISHLIST ITEMS SAVED</h3><p className="text-xs text-gray-600 font-bold uppercase tracking-wider mb-6">
