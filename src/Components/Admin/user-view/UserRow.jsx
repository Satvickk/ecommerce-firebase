...
export default function UserRow({ responseData }) {
  ...
  return <><tr className="border-b border-black last:border-b-0 hover:bg-swiss-muted text-xs font-bold uppercase tracking-wider text-black"><td className="p-4 font-black">{responseData?.name}</td><td className="p-4">{responseData?.email}</td><td className="p-4">{responseData?.contact}</td><td className="p-4 font-mono text-[10px]">••••••••</td><td className="p-4">{responseData?.pincode}</td><td className="p-4">{responseData?.address}</td><td className="p-4 flex gap-2"><button
  ...
  >
            DELETE
          </button></td></tr>{showDeleteModal && responseData?.docId && <DeleteModal onClose={closeDeleteModal} id={responseData.docId} />}{showUpdateModal && <UpdateUserProfile onClose={closeUpdateModal} userData={responseData} />}</>;
}
export function DeleteModal({ onClose, id }) {
  const [loading, setLoading] = useState(false);
  const dispatch = useAppDispatch();
  const handleDeleteUserProfile = async (id2) => {
    setLoading(true);
    try {
      const result = await USER_SERVICE.deleteUser(id2);
      if (result) {
        dispatch(removeSingleUserDetails(id2));
  ...
  return <dialog open className="modal modal-bottom sm:modal-middle"><div className="modal-box border-4 border-black bg-white rounded-none p-0 max-w-md w-full"><div className="bg-black text-white p-4 border-b-4 border-black"><h3 className="font-black text-sm uppercase tracking-widest text-swiss-accent">DELETE USER ACCOUNT</h3></div><div className="p-6 space-y-4"><p className="text-xs font-bold uppercase tracking-wider text-black leading-relaxed border-l-4 border-swiss-accent pl-3">
  ...
  >
              CONFIRM DELETE
            </LoadingButton><button className="w-full bg-white text-black font-black text-xs uppercase tracking-widest py-3 border-2 border-black hover:bg-swiss-muted" onClick={onClose}>
              CANCEL
