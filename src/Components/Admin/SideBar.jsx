...
export default function SideBar({ tabs }) {
  return <><div className="sm:flex flex-col w-full gap-3 hidden"><h3 className="text-xs font-black uppercase tracking-widest text-black border-b-2 border-black pb-3 mb-2">
          01. ADMIN NAVIGATION
        </h3>{tabs.map((item) => <NavLink
    key={item.label}
  ...
  >{item.label}</NavLink>)}</div><MobileAdminMenu tabs={tabs} /></>;
}
export function MobileAdminMenu({ tabs }) {
  ...
  return <div className="flex sm:hidden flex-col w-full gap-2"><h3 className="text-xs font-black uppercase tracking-widest text-black">
        NAVIGATION MENU
      </h3><select
  ...
  >{tabs.map((item) => <option key={item.label} value={item.src}>{item.label}</option>)}</select></div>;
}
