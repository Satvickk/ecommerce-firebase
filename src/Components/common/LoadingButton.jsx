export default function LoadingButton({ isLoading, className = "", children, ...props }) {
  ...
  >{isLoading ? <span className="flex items-center justify-center gap-2"><span className="animate-spin border-2 border-white border-t-transparent w-4 h-4 rounded-full" /><span>PROCESSING...</span></span> : children}</button>;
}
