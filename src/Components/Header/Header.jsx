...
export default function Header() {
  ...
  return <header className="w-full bg-white border-b-4 border-black sticky top-0 z-50"><div className="max-w-7xl mx-auto px-4 sm:px-8 h-20 flex items-center justify-between"><div className="flex items-center gap-8"><Logo />{
    /* Main Desktop Navigation */
  }<nav className="hidden lg:flex items-center gap-6 ml-6"><NavLink
    to="/"
    className={({ isActive }) => `text-xs font-bold uppercase tracking-widest px-3 py-2 transition-colors duration-150 ${isActive ? "bg-black text-white" : "text-black hover:bg-swiss-muted"}`}
  >
              01. Home
            </NavLink><NavLink
    to="/product"
  ...
  >
    ...
    /* Right Action Items */
  }{isLoggedIn ? <div className="hidden sm:flex items-center gap-3">{userData?.userRole === 2 && <Link
    to="/admin"
  ...
  >
                Admin Console
              </Link>}<div className="dropdown dropdown-end"><div
    tabIndex={0}
    role="button"
  ...
  ><span>Menu</span><span className="text-swiss-accent font-black">↓</span></div><ul
    tabIndex={0}
    className="dropdown-content border-2 border-black bg-white z-50 w-56 p-0 mt-1 shadow-none rounded-none"
  >{PHONE_MENU_DATA.map((item) => <li key={item.label} className="border-b border-black last:border-b-0"><NavLink
    className={({ isActive }) => `block px-4 py-3 text-xs font-bold uppercase tracking-widest transition-colors duration-150 ${isActive ? "bg-black text-white" : "text-black hover:bg-swiss-accent hover:text-white"}`}
    to={item.link}
  >{item.label}</NavLink></li>)}<li className="border-t-2 border-black"><button
  ...
  ><span>Cart</span>{UserCart?.selectedProducts?.length > 0 && <span className="bg-swiss-accent text-white px-2 py-0.5 text-[10px] font-black border border-white">{UserCart?.selectedProducts.length}</span>}</label></div> : <div className="hidden sm:flex items-center gap-3"><NavLink
    to="/login"
