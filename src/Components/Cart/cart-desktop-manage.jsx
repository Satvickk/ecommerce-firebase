...
export default function CartDesktopManage() {
  ...
  return <><h1 className="divider text-md sm:text-xl my-8 font-medium">
        Cart
        <img src={"/cart.svg"} alt={"cart"} className="w-6 h-6 inline-block" /></h1><div className="flex flex-col gap-4">{UserCart?.selectedProducts?.map((item, index) => <CartCard key={item.docId || index} data={item} />)}</div>{UserCart.totalCost !== 0 ? <LoadingButton
    isLoading={loading}
    className="btn btn-primary w-full my-4"
    onClick={handleCheckout}
  >
          Checkout ₹ {UserCart.totalCost}</LoadingButton> : <p className="text-center">No Products in Cart</p>}</>;
}
