...
export default function UpdateProfile({ handleClose, isClose = true }) {
  ...
  return <div className="space-y-6"><div className="border-b-4 border-black pb-3"><h2 className="text-xl font-black uppercase tracking-tight text-black">UPDATE USER SPECIFICATIONS</h2></div><form className="space-y-4" onSubmit={handleSubmit(UpdateUserDetails)}><div><label className="block text-xs font-black uppercase tracking-widest text-black mb-1">FULL NAME</label><input
    type="text"
  ...
  /><p className="text-swiss-accent text-xs font-bold mt-1">{errors.contact?.message}</p></div><div><label className="block text-xs font-black uppercase tracking-widest text-black mb-1">POSTAL PIN CODE</label><input
    ...
    className="w-full border-2 border-black px-4 py-3 text-xs font-bold uppercase focus:border-swiss-accent focus:outline-none rounded-none"
    {...register("pincode")}
  /><p className="text-swiss-accent text-xs font-bold mt-1">{errors.pincode?.message}</p></div></div><div className="pt-4 flex flex-col sm:flex-row gap-3"><LoadingButton isLoading={loading} type="submit" className="w-full py-3">
            SAVE UPDATES →
          </LoadingButton>{handleClose && <button
    type="button"
